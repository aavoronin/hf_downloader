...
class TextToTextModelFactory:
    def __init__(self, root_folder: str):
        ...
        self._error_logs: Dict[str, TextToTextModelErrorLog] = {}
        self._load_config()
        ...
        if self.config_path.exists():
            try:
                ...
                for name, log_data in data.get('error_logs', {}).items():
                    self._error_logs[name] = TextToTextModelErrorLog.from_dict(log_data)
    ...
    def _log_error(self, model_name: str, error_message: str):
        if model_name not in self._error_logs:
            self._error_logs[model_name] = TextToTextModelErrorLog(model_name=model_name)
    ...
    def get_error_count(self, model_name: str) -> int:
        return self._error_logs.get(model_name, TextToTextModelErrorLog(model_name=model_name)).error_count
    ...
    def list_available_models(self) -> List[TextToTextModelInfo]:
        ...
        for item in self.root_folder.iterdir():
            ...
            models.append(TextToTextModelInfo(
                name=model_name, folder_name=item.name, size_bytes=total_size,
                size_human=self._format_size(total_size), path=item, files=files
            ))
    ...
    def save_statistics(self, stats: Dict[str, Any]):
        ...
        existing['processing_history'].append({
            'timestamp': datetime.now().isoformat(), **stats
        })
        try:
